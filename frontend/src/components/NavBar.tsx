...
export function NavBar() {
    ...
    return (
        <div className={`flex justify-evenly text-white w-full bg-gradient-to-r from-blue-400 to-[#81b1ce] text-md font-semibold p-4`}>
            ...
            <button onClick={handleGenerate} className="cursor-pointer w-full flex">Generate</button>
            <div className="cursor-default flex justify-center w-full">{user?.email}</div>
            <button onClick={handleLogOut} className="cursor-pointer flex justify-end items-center space-x-1 w-full">
                <div>Log Out</div>
